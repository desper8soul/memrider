...
export class LlmService {
  ...
  constructor(private readonly config: ConfigService) {
    ...
    const bearerToken = this.config.get<string>('AWS_BEARER_TOKEN_BEDROCK');
    ...
    if (region && bearerToken) {
      ...
      this.logger.log(`Bedrock client ready (region=${region}, bearer auth)`);
    } else if (region) {
      this.client = new BedrockRuntimeClient({ region });
      this.logger.log(`Bedrock client ready (region=${region}, default auth)`);
    }
  }
  ...
  async synthesize(input: LlmSynthesisInput): Promise<{
    ...
    const modelId =
      this.config.get<string>('BEDROCK_MODEL_ID') ?? 'amazon.nova-lite-v1:0';
  ...
  private mockResponse(userPrompt: string): string {
    this.logger.warn('Bedrock unavailable — using deterministic mock LLM');
